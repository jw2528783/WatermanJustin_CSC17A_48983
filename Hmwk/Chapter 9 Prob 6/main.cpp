...
using namespace std;
 ...
 int main(){
     ...
     int *donations= new int[NUM_DONATIONS];//Dynamically allocate "donations" array.
     fillArray(donations,NUM_DONATIONS);//Lets user input the donations.
     ...
     delete [] donations;//Release dynamically allocated memory.
     return 0;
 }
void fillArray(int *donations, int NUM_DONATIONS){
    cout<<"Enter the donations made by the employes of CK Graphics, Inc."<<endl;
    for(int i=0; i<NUM_DONATIONS; i++)cin>>*(donations+i);
}
 //****************************************************************
